    ...
    <Fragment>
      <section id="home" className="relative items-center min-h-screen bg-black overflow-hidden mx-auto max-w-9xl pt-8 md:pb-8">
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div data-aos="zoom-out" className="absolute bottom-10 right-10 sm:right-20 w-24 sm:w-28 h-24 sm:h-28 rounded-full bg-red-600 opacity-20 blur-3xl animate-float-delay"></div>
        </div>

        <div className="flex flex-col-reverse md:flex-row items-center gap-12 w-full mx-auto max-w-8xl z-10 py-15 md:py-20 p-4 md:p-5 md:ml-16">
          ...
          <div className="w-full md:w-1/2 text-center md:text-start">
            ...
            <div className="flex flex-row gap-3 sm:gap-4 mb-10 justify-center sm:justify-start">
              <button data-aos="fade-down" className="relative bg-red-600 text-white px-4 py-2 sm:px-6 sm:py-3 rounded-full font-semibold text-sm sm:text-base flex items-center gap-2 transition-all duration-500 transform hover:scale-110 shadow-lg overflow-hidden group">
              ...
              <button data-aos="fade-up" className="relative border-2 border-red-600 text-red-600 px-4 py-2 sm:px-6 sm:py-3 rounded-full font-semibold text-sm sm:text-base flex items-center gap-2 transition-all duration-500 transform hover:scale-110 hover:bg-red-600 hover:text-white shadow-md group">
            ...
            {/* Social Icons */}
            <div className="mt-8">
              <h2 data-aos="fade-down" className="text-white mb-4 tracking-wide text-sm sm:text-base">🌐 Connect with me</h2>
              <div className="flex justify-center md:justify-start items-center gap-4">
                {socialLinks.map((item, index) => (
                  <button
                    ...
                    className={`text-white cursor-pointer border flex justify-center items-center bg-black ${item.color} h-10 w-10 sm:h-12 sm:w-12 rounded-full transition-all duration-500 transform hover:scale-110`}
                  >
                    {item.icon}
                  </button>
                ))}
        ...
        <style jsx global>{`
          ...
          @keyframes pulse-delay {0%,100%{opacity:0.2;transform:scale(1);}50%{opacity:0.4;transform:scale(1.1);} }
