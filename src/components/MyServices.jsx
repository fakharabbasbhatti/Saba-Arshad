  ...
  return (
    <Fragment>
      <div className="pt-8 pb-8 border-t-2 border-red-500">
        <section id="services" className="relative mx-auto max-w-7xl bg-black overflow-hidden px-4 sm:px-6 lg:px-8">
          {/* Background shapes */}
          ...
          <div className="text-center relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {services.map((service, index) => (
                <div key={index} className="group relative overflow-hidden bg-black border border-red-600 rounded-2xl p-5 md:p-6 h-full transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-red-600/30">
                  <div className="relative z-10 flex flex-col items-center text-center">
                    <div data-aos={service.aosIcon} data-aos-delay={index * 100} className="w-16 h-16 mb-4 flex items-center justify-center rounded-2xl bg-red-600 group-hover:bg-white transition-all duration-500">
                      <span className="text-white group-hover:text-red-600 text-3xl transition-transform duration-500">{service.icon}</span>
                    </div>
                    <h3 data-aos={service.aosTitle} data-aos-delay={index * 150} className="text-xl sm:text-2xl font-semibold text-white mb-2">{service.title}</h3>
                    <p data-aos={service.aosDesc} data-aos-delay={index * 200} className="text-gray-300 text-sm sm:text-base">{service.description}</p>
                  </div>
                </div>
              ))}
            </div>
